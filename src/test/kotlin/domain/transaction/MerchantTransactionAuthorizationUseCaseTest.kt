...
class MerchantTransactionAuthorizationUseCaseTest {
    private lateinit var useCase: MerchantTransactionAuthorizationUseCase
    ...
    fun initAll() {
        ...
        this.useCase = MerchantTransactionAuthorizationUseCase(transactionRepository, walletRepository)
    }
    ...
    @Test
    fun `should sub balance from wallet based on merchantName and return approved transaction`() = runTest {
        ...
        val wallet = newWallet(BalanceType.MEAL, mockedAccountId).addBalance(10000)
        walletRepository.save(wallet)

        val input = TransactionAuthorizationInput(mockedAccountId, 1500, "1234", "UBER EATS     SAO PAULO")
    ...
    @Test
    fun `should sub balance from wallet based on MCC when not exists a rule to merchant name`() = runTest {
        val mockedAccountId = "3894eae4-216f-451a-b5a8-ca361a3597d7"
        val mockedWalletId = "ac86166f-2463-46c8-afa7-1915aa027170"
        every { UUID.randomUUID() } returns UUID.fromString(mockedWalletId)
        val wallet = newWallet(BalanceType.CASH, mockedAccountId).addBalance(10000)
        walletRepository.save(wallet)

        val input = TransactionAuthorizationInput(mockedAccountId, 1500, "1234", "MERCHANT WITHOUT RULE")
        val result = useCase.execute(input)
    ...
    fun `should return insufficient balance when transaction amount is greater than wallet balance`() = runTest {
        ...
        val input = TransactionAuthorizationInput(mockedAccountId, 15000, "5411", "IFOOD")
        val result = useCase.execute(input)
